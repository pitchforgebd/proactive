 ...
 * Logos always show in full colour (no grayscale / hover colour swap).
 */
    ...
    >
      {partners.map((p) => (
        <li key={`${p.id}-${hidden}`} className="shrink-0">
          <div className="flex h-[72px] w-[148px] items-center justify-center rounded-xl border border-ink/10 bg-paper-2 px-4 dark:border-line dark:bg-band-2">
            <Image
              src={p.logo}
              alt={hidden ? '' : p.name}
              width={160}
